...
"""
        ...
        prompt = f"""你是专业的 Python 代码生成和执行专家。
...
1. **小步执行**：不要一次性写完所有代码，分步骤执行
2. **必须 print**：每次 `run_python` 都要用 print() 查看结果
3. **使用实际列名**：{actual_columns}
4. **保存到指定路径**：严格使用上面指定的文件路径
5. **🔥 只保存必要的列**：保存结果时必须只选择 ID 列和新生成的列，不要保存整个 DataFrame
5. **处理依赖**：如果需要前一步的结果，先检查文件是否存在，再加载
...
**🛑 停止条件：**
当你使用 `read_file` 验证结果文件存在且格式正确后，**立即停止并返回最终答案**。
不要执行任何额外的测试、分析或验证代码。
...
开始执行吧！记住：小步快跑，每步都要 print 确认！完成验证后立即返回最终答案！"""
